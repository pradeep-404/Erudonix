...
export async function POST(request: Request) {
  ...
  try {
    ...
    const checkUser = await dbClient.query('SELECT 1 FROM public.users WHERE email = $1', [email])
    ...
    if (isSmtpConfigured) {
      // Send real email
      const mailOptions = {
        from: `"Erudogix Verification" <${process.env.SMTP_USER}>`,
        to: email,
        subject: `${otpCode} is your Erudogix Verification Code`,
        html: `
          <div style="font-family: sans-serif; background-color: #0d0e12; color: #f3f4f6; padding: 40px; border-radius: 16px; max-width: 600px; margin: 0 auto; border: 1px solid #1f2937;">
            ...
            <div style="background-color: #111827; border: 1px solid #374151; padding: 30px; border-radius: 12px; text-align: center;">
              <h2 style="margin-top: 0; color: #ffffff; font-size: 22px;">Verify your email address</h2>
              <p style="color: #9ca3af; font-size: 14px; margin-bottom: 24px;">Please use the following 6-digit verification code to complete your registration. This code is valid for 10 minutes.</p>
              ...
              <p style="color: #6b7280; font-size: 12px; margin-top: 20px;">If you did not request this code, you can safely ignore this email.</p>
            </div>
      ...
      // Background SMTP delivery to make API request return instantly
      transporter.sendMail(mailOptions)
        .then(() => console.log(`[OTP Sent Successfully] Verification code sent to ${email}`))
        .catch((mailErr) => console.error(`[Background SMTP delivery failed to ${email}]:`, mailErr))
    } else {
      // Print in console for local dev environment
      console.log('\n==================================================')
      console.log(`[DEV OTP SERVICE] Verification OTP code for ${email} is:`)
      console.log(`\n       --->  ${otpCode}  <---\n`)
    ...
    // In development mode, return devOtp for convenience if SMTP is not configured
